...
const doEverything = async () => {
  ...
  const pestoRecipe = await Recipe.create({
    title: "Pesto Pasta",
    level: "Easy Peasy",
  ...
  console.log(pestoRecipe.title, "created");
  ...
  console.log("Successfully updated", updateRigatoni);
  const deleteCarrot = await Recipe.deleteOne({ title: "Carrot Cake" });
  console.log("Successfully deleted", deleteCarrot);
  const closeConnection = await mongoose.connection.close();
  console.log("successfully closed", closeConnection);
};
